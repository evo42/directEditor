...
(function ($) {
	...
	$.widget("directEdit.directImageEditor", {
		...
		options: {
			...
			scaleType : 'normal',
			dataAttrContainer: null  // reference to the dataAttr of the container
		},
		_create: function () {
			...
			if ($.directEdit.directManager && this.options.useManager && this.isImage) {
				this.instanceID = $.directEdit.directManager.register(this, this.element.attr('id'));
				this.dataAttr = $.directEdit.directManager.getData(this.element);
				this.dataAttrReset = this.dataAttr;
			}
					...
					if (self.isImage) {
						...
						if ($(event.target).is(self.options.selector)) {
							...
							if ($.directEdit.directManager) {
								self.dataAttr = {};
								$.extend(self.dataAttr, self.options.dataAttrContainer, $.directEdit.directManager.getData(self.image));
								self.dataAttrReset = self.options.dataAttrContainer;
							}
						} else {
							self.image = null;
							self.imageData = {};
							if ($.directEdit.directManager) {
								self.dataAttr = self.options.dataAttrContainer;
								self.dataAttrReset = self.dataAttr;
							}
						}
					}
				};
		...
		_createSuccessHandler: function () {
			...
			this.callback = function (result) {
				if (self.options.callback) {
					...
					if ($.directEdit.directManager && self.isImage) {
						$.directEdit.directManager.unRegister(self.image.attr('id'));
					}
					self.image
						.after(result.content)
						.next()
						.directImageEditor(self.options) // a fresh and new editor
						.directImageEditor('setModified'); // let directManager save it
					self.image.remove();
				}
			};
		},
		...
		_createEditPage: function () {
			var selectedStyle, style, styles, isFirstStyle, contentEdit, imageTypeButtons,
				editableImage, sliderContainer, editorOptions, spinner, toolBox, editForm, buttonSet,
			...
			buttonSet = $('<div class="buttonset"><input type="button" id="imageReset" value="Andere afbeelding" /><input type="button" id="sendForm" value="Save"></div>');
			...
			(function () {
				...
				saveImageData = function () {
					var saveData  = {data: {}};
					$.extend(saveData.data, self.dataAttr, editableImage.directImageEditorCPS('getState'));
			...
			if (this.imageData.file) {
				...
				this.saveSourceImage(this.dataAttrReset, this.imageData.file, saveSourceImageSuccess);
			}
		...
		_createUploadPage: function () {
			...
			contentUpload.find(':file').change(function () {
				...
				if (!FileReader) {
					...
					saveSourceImageSuccess = function (ret) {
						if (ret.data) { $.extend(self.dataAttr, ret.data); }
						if (ret.url) { self.imageData.source = ret.url; }
						self._createEditPage();
					};
					$('<div id="spinner"></div>').appendTo(contentUpload).show();
					self.saveSourceImage(self.dataAttr, file, saveSourceImageSuccess);
				} else {
					...
					reader.onerror = function () { window.alert('Fout bij lezen afbeelding.'); };
		...
		_createConfirmPage: function (imageData, file) {
				...
				if (self.options.scaleType === 'normal') {
					...
					if ((self.options.constraints.minWidth && image.width() < self.options.constraints.minWidth) && (self.options.constraints.minHeight && image.height() < self.options.constraints.minHeight)) {
						if (self.options.constraints.minWidth && image.width() < self.options.constraints.minWidth) {
							contentConfirm.append('<p>The image width is too small, it should be ' + self.options.constraints.minWidth + 'px and it is ' + image.width() + 'px.</p>');
						}
						if (self.options.constraints.minHeight && image.height() < self.options.constraints.minHeight) {
							contentConfirm.append('<p>The image height is too small, it should be ' + self.options.constraints.minHeight + 'px and it is ' + image.height() + 'px.</p>');
						}
						return false;
					}
				}
			...
			afterImageLoad = function () {
				...
				$('<input type="button" id="imageReset" value="Andere afbeelding" />').appendTo(buttonset)
				...
				if (checkImageSize(thumb)) {
					...
					thumb.show();
					$('<input type="button" id="imageUploadButton" value="Uploaden en bewerken >>" />').appendTo(buttonset)
						.click(function (event) {
							// prevent the dialog from being closed by the pseudo-blur detector
							event.stopPropagation();
							self.imageData = {};
		...
		saveSourceImage: function (dataAttr, file, callback) {
			var formData = new FormData(), dataElement;
			formData.append('file', file);
			formData.append(this.options.uploadCommand[0], this.options.uploadCommand[1]);
			for (dataElement in dataAttr) {
				if (dataAttr.hasOwnProperty(dataElement) && dataElement !== 'file') {
					formData.append('data[' + dataElement + ']', dataAttr[dataElement].toString());
				}
			}
			$.ajax({
				url: this.options.ajaxUrl || (($.directEdit.directManager) ? $.directEdit.directManager.getAjaxUrl() : ''),
				type: 'POST',
				...
				//Ajax events
				error: function () {
					window.alert('Kan afbeelding niet uploaden');
		...
		saveImageData: function (data, callback) {
			$.ajax({
				url: this.options.ajaxUrl || (($.directEdit.directManager.getAjaxUrl) ? $.directEdit.directManager.getAjaxUrl() : ''),
				type: 'POST',
				error: function () {
					window.alert('Kan bewerking niet opslaan');
				},
				dataType: 'json',
				success: callback,
				data: data
		...
		setData: function (data) {
			var newImage;
			if (this.isImage) {
				if ($.directEdit.directManager.unRegister) {
					$.directEdit.directManager.unRegister(this.image.attr('id'));
				}
				newImage = this.image
					.after(data.content)
					.next()
				...
				// todo generalize for use with container
				console.log('setData for images in a container not implemented. Use container logic instead');
			}
		}
	});

/*
 * directImageEditorCPS a $ UI widget for image editting, part of the directEdit project
 * (c) 2010-2012 Carlo Roosen (http://www.carloroosen.nl)
 */
